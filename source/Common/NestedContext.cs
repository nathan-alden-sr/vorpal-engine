...
public readonly struct NestedContext
{
    ...
    public NestedContext Push(string newContext)
        => new(Description, newContext, _separator);
    ...
    /// <param name="expandGenericArguments">A value indicating whether to include generic type arguments.</param>
    /// <returns>A new <see cref="NestedContext" /> that is the combination of the current context and the new context.</returns>
    public NestedContext Push<T>(bool fullName = false, bool expandGenericArguments = true)
        => Push(typeof(T), fullName, expandGenericArguments);
    ...
    public static NestedContext None(string separator = DefaultSeparator)
        => new(null, separator);
}