...
#pragma warning restore 649
    ...
    /// <param name="groupIndex">The index of the group to set.</param>
    public void BitwiseComplement(byte groupIndex)
        => Set(groupIndex, (byte)(~Get(groupIndex) & GroupMask));
    ...
    public void LeftShift(byte groupIndex, int shift)
        => Set(groupIndex, (byte)((Get(groupIndex) << shift) & GroupMask));
    ...
    public void RightShift(byte groupIndex, int shift)
        => Set(groupIndex, (byte)((Get(groupIndex) >> shift) & GroupMask));

    /// <summary>Sets a group of bits by performing a logical AND (<c>&amp;</c>) with the current bits and <paramref name="value" />.</summary>
    /// <param name="groupIndex">The index of the group to set.</param>
    /// <param name="value">
    ///     A value containing the bits to logical AND (<c>&amp;</c>) on the current bits. Bits not in <see cref="GroupMask" /> will be
    ///     discarded.
    /// </param>
    public void LogicalAnd(byte groupIndex, byte value)
        => Set(groupIndex, (byte)(Get(groupIndex) & value));

    /// <summary>Sets a group of bits by performing a logical AND (<c>|</c>) on the current bits and <paramref name="value" />.</summary>
    /// <param name="groupIndex">The index of the group to set.</param>
    /// <param name="value">
    ///     A value containing the bits to logical OR (<c>|</c>) with the current bits. Bits not in <see cref="GroupMask" /> will be
    ///     discarded.
    /// </param>
    public void LogicalOr(byte groupIndex, byte value)
        => Set(groupIndex, (byte)(Get(groupIndex) | value));
    ...
    /// <param name="groupIndex">The index of the group to set.</param>
    /// <param name="value">
    ///     A value containing the bits to logical exclusive OR (<c>^</c>) on the current bits. Bits not in <see cref="GroupMask" /> will
    ///     be discarded.
    /// </param>
    public void LogicalExclusiveOr(byte groupIndex, byte value)
        => Set(groupIndex, (byte)(Get(groupIndex) ^ value));
    ...
    private static int GetFieldIndex(byte groupIndex)
        => groupIndex * BitsPerGroup / BitsPerField;

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static int GetShift(byte groupIndex)
        => groupIndex % GroupsPerField * BitsPerGroup;

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static byte PrepareValue(byte value)
        => (byte)(value & GroupMask);
}