...
public struct KeyboardStateChanges
{
    ...
    public bool IsKeyDown(Key key)
        => (_stateBitmap.Get((byte)key) & KeyboardManager.KeyDownStateMask) != 0;
    ...
    public bool WasKeyPressed(Key key)
        => (_stateBitmap.Get((byte)key) & KeyboardManager.KeyPressedStateMask) != 0;
    ...
    public bool WasKeyReleased(Key key)
        => (_stateBitmap.Get((byte)key) & KeyboardManager.KeyReleasedStateMask) != 0;

    internal byte Get(byte groupIndex)
        => _stateBitmap.Get(groupIndex);

    internal void Set(byte groupIndex, byte value)
        => _stateBitmap.Set(groupIndex, value);

    internal void LogicalAnd(byte groupIndex, byte value)
        => _stateBitmap.LogicalAnd(groupIndex, value);
}
